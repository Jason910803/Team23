...
def perform_vector_search(query_text, limit=10, num_candidates=None):
    print(f"Performing vector search with query: {query_text}")
...
def fuzzy_search(request):
    ...
    if query:
        ...
        MONGO_URI = os.getenv(
            "MONGO_URI", "mongodb://localhost:61534/?directConnection=true"
        )
        ...
        if "recipes" not in db.list_collection_names():
            raise Http404("Collection 'recipes' does not exist in the database.")
        else:
            print("Collection 'recipes' exists.")
        ...
        print("number of recipe IDs:", len(recipe_ids))
        ...
        print("number of recipes:", len(recipes_unordered))
        ...
        print("number of recipes after sorting:", len(recipes))

    # Render the template with results
    return render(request, "fuzzy_search.html", {"recipes": recipes, "query": query})
